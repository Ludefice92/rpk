...
int  sum (int count,...) {
  ...
  va_start(args, count);  // Initialize the va_list with the last fixed argument
...
int min(int count,...) {
  int min = 2147483647; //max positive int value
  int curVal = 0;
  va_list args;  // Declare a va_list variable to hold the arguments
  va_start(args, count);  // Initialize the va_list with the last fixed argument
  for(int i=0; i<count; i++) {
      curVal = va_arg(args,int);
...
int max(int count,...) {
  int max = -2147483648; //lowest int value
  int curVal = 0;
  va_list args;  // Declare a va_list variable to hold the arguments
  va_start(args, count);  // Initialize the va_list with the last fixed argument
  for(int i=0; i<count; i++) {
