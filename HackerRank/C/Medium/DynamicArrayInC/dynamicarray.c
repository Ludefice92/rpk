...
int main()
{
    ...
    // Allocate memory for the total number of books and pages
    total_number_of_books = (int*)malloc(total_number_of_shelves * sizeof(int));
    total_number_of_pages = (int**)malloc(total_number_of_shelves * sizeof(int*));
    
    // Initialize the arrays
    for (int i = 0; i < total_number_of_shelves; i++) {
        total_number_of_books[i] = 0;  //no books yet
        total_number_of_pages[i] = NULL;  //no pages until a book is added
    }
    ...
    while (total_number_of_queries--) {
        ...
        if (type_of_query == 1) {
            ...
            int x, y;
            scanf("%d %d", &x, &y);
            int numOfBooks = total_number_of_books[x];
            ...
            total_number_of_pages[x][numOfBooks] = y;
